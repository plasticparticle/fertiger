...
#      IN_DEVELOPMENT, QA_REVIEW, CODE_REVIEW, SECURITY_REVIEW, READY_FOR_MERGE, DONE
#
...
if [ -z "$OPTION_ID" ]; then
  ...
  echo "Available statuses: BACKLOG INTAKE LEGAL_REVIEW ARCHITECTURE SOLUTION_DESIGN AWAITING_APPROVAL" >&2
  echo "                    IN_DEVELOPMENT QA_REVIEW CODE_REVIEW SECURITY_REVIEW READY_FOR_MERGE DONE" >&2
  exit 1
fi
